...
void ServerConnection::saveSession(const QString &myUser,
                                   const QString &friendUser)
{
    if (!isLoggedIn) return;

    QJsonObject json;
    json["type"] = "save_session";
    json["myUser"] = myUser;
    json["friendUser"] = friendUser;
...
void ServerConnection::getFriends()
{
    ...
    json["user_id"] = currentUser.getId();
...
void ServerConnection::onTextMessageReceived(const QString &message)
{
    ...
    {
        QString sender = obj["sender_user"].toString();
        QString receiver = obj["receiver_user"].toString();
        QString content = obj["content"].toString();
        QString sentAt = obj["sent_at"].toString();

        // Prima notificare pentru propriul client a fost deja adăugată local
        if (sender == currentUser.getUsername()) {
            return;
        }

        QJsonObject m;
        m["sender_user"] = sender;
        m["receiver_user"] = receiver;
        m["content"] = content;
        m["sent_at"] = sentAt.isEmpty() ? QDateTime::currentDateTime().toString(Qt::ISODate) : sentAt;
        m["from_server"] = true;
